...
import { Shield, Lock, User as UserIcon, Loader2, Info, KeyRound } from 'lucide-react';
...
const Login: React.FC<LoginProps> = ({ onLogin, onToggle }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    showToast("Verifying identity with Atlas...", "loading");
    ...
    try {
      ...
      showToast('Connection to Atlas timed out. Check IP Whitelist.', 'error');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0f172a] p-6 relative">
      ...
      <div className="w-full max-w-md animate-in fade-in duration-300">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white">Enterprise Access</h1>
          <p className="text-slate-400 mt-2">Secure verification with MongoDB Atlas</p>
        </div>

        <div className="bg-[#1e293b] border border-slate-800 rounded-3xl p-8 shadow-2xl">
          <form onSubmit={handleLogin} className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-2">Identity</label>
          ...
          <div className="mt-6 text-center">
            ...
            >
              No account? <span className="text-blue-400 font-bold">Register here</span>
            </button>
          </div>
        </div>
      </div>
